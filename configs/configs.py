...
data_dir  =os.path.join(os.environ['PKG_PATH'], os.environ['PKG_NAME'], 'dataset')

dir_raw = os.path.join(str(Path(os.environ['PKG_PATH']).parent), 'data_df', 'raw_with_raster', 'scene_df')
if not os.path.isdir(dir_raw):
    os.makedirs(dir_raw)

dir_filter = os.path.join(data_dir, 'filtered', 'scene_df')
if not os.path.isdir(dir_filter):
    os.makedirs(dir_filter)

dir_processed = os.path.join(data_dir, 'processed', 'scene_df')
...
processed_data_config = {
    ...
    'freq':2,
    'num_closest_obs': 6,
    'filtered_data_dir': dir_filter,
    'data_save_dir': dir_processed,
    'num_workers': NUM_WORKERS
}
