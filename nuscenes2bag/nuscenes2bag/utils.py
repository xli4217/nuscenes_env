...
class Utils(object):
    ...
    def __init__(self, nusc, nusc_can):
        self.nusc = nusc
    ...
    def get_categories(self,first_sample):
        categories = set()
        sample_lidar = first_sample
        while sample_lidar is not None:
            sample = self.self.nusc.get('sample', sample_lidar['sample_token'])
            for annotation_id in sample['anns']:
                ann = self.nusc.get('sample_annotation', annotation_id)
                categories.add(ann['category_name'])
            sample_lidar = self.nusc.get('sample_data', sample_lidar['next']) if sample_lidar.get('next') != '' else None
        return categories
    ...
    def get_camera_info(self,sample_data, frame_id):
        ...
        msg_info.header.stamp = self.get_time(sample_data)
    ...
    def get_lidar_imagemarkers(self,sample_lidar, sample_data, frame_id):
        ...
        marker.header.stamp = self.get_time(sample_data)
        marker.ns = 'LIDAR_TOP'
        marker.id = 0
    ...
    def write_boxes_imagemarkers(self,bag, anns, sample_data, frame_id, topic_ns, stamp):
        # annotation boxes
        collector = Collector()
        _, boxes, camera_intrinsic = self.nusc.get_sample_data(sample_data['token'])
        for box in boxes:
        ...
        marker.action = ImageMarker.ADD
        marker.scale = 2.0
        marker.points = [self.make_point2d(p) for p in collector.points]
        marker.outline_colors = [self.make_color(c) for c in collector.colors]
    ...
    def write_occupancy_grid(self,bag, nusc_map, ego_pose, stamp):
    ...
    def get_imu_msg(self,imu_data):
        msg = Imu()
        msg.header.frame_id = 'base_link'
        msg.header.stamp = self.get_utime(imu_data)
    ...
    def get_odom_msg(self,pose_data):
        ...
        msg.header.stamp = self.get_utime(pose_data)
    ...
    def find_closest_lidar(self, lidar_start_token, stamp_nsec):
        ...
        while next_lidar_token != '':
            ...
            dist_abs = abs(stamp_nsec - self.get_time(lidar_data).to_nsec())
