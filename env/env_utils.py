...
def get_discretized_future_lanes(ego_pos, ego_quat, lane_record, nusc_map, frame='global'):
    ...
    lane_discretized_pos_global = np.array(arcline_path_utils.discretize_lane(lane_record, resolution_meters=1))[:,:2]
...
def get_future_lanes(nusc_map, ego_pos, ego_quat, frame='global'):
    ## lanes ##
    # closest lane
    ...
    for idx, lane in enumerate(outgoing_lane_ids):
        ...
        outgoing_lane_local = get_discretized_future_lanes(ego_pos, ego_quat, lane_record, nusc_map, frame=frame)
