...
def final_distances(stacked_trajs: np.ndarray, stacked_ground_truth: np.ndarray) -> np.ndarray:
    ...
    :param stacked_trajs: Array of shape [batch_size, num_modes, horizon_length, state_dim].
    ...
    diff_of_last = np.take(stacked_trajs, [-1], -2).squeeze(-2) - np.take(stacked_ground_truth, [-1], -2).squeeze(-2)
    return np.mean(np.linalg.norm(diff_of_last, axis=-1), axis=0)
