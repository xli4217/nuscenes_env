...
class NuScenes:
    ...
    def render_sample(self, sample_token: str, box_vis_level: BoxVisibility = BoxVisibility.ANY, nsweeps: int = 1,
        ...
        fig, ax = self.explorer.render_sample(sample_token, box_vis_level, nsweeps=nsweeps, out_path=out_path)
        return fig, ax
        
    def render_sample_data(self, sample_token:str=None, sample_data_token: str=None, with_anns: bool = True,
        ...
        fig, ax = self.explorer.render_sample_data(sample_token, sample_data_token, with_anns, box_vis_level, axes_limit, ax, nsweeps=nsweeps,
    ...
    def render_annotation(self, sample_annotation_token: str, margin: float = 10, view: np.ndarray = np.eye(4),
                          ...
                          extra_info: bool = False, render_lidar: bool = False):
        return self.explorer.render_annotation(sample_annotation_token, margin, view, box_vis_level, out_path, extra_info, render_lidar=render_lidar)
...
class NuScenesExplorer:
    ...
    def render_ego_centric_map(self,
        ...
        map_mask = map_['mask']
        #nusc_map = NuScenesMap(dataroot=self.nusc.dataroot, map_name=log['location'])
    ...
    def render_sample_data(self,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            r_img = rotate(robot, angle=0, axes=(1,0))
            oi = OffsetImage(r_img, zoom=0.015, zorder=700)
            veh_box = AnnotationBbox(oi, (0,0), frameon=False)
            veh_box.zorder = 700
        ...
        # ax.axis('off')
        ax.set_title('scene: '+scene_name + ', sample: ' + sample_token +', channel: '+sd_record['channel'] + '\n' + ', description: '+description)
        ax.set_aspect('equal')
        ax.grid(False)
