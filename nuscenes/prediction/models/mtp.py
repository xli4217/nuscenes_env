...
from external_libs.nuscenes.prediction.models.backbone import calculate_backbone_feature_dim
...
class MTP(nn.Module):
    ...
    def __init__(self,
        ...
        self.fc1 = nn.Linear(backbone_feature_dim + ASV_DIM + noise_dim, n_hidden_layers)
        ...
        self.env_feature_dim = backbone_feature_dim + ASV_DIM
        self.env_feature = None
        
    def get_env_feature(self):
        return self.env_feature
...
class MTPLoss(object):
    ...
    def __call__(self, predictions: torch.Tensor, targets: torch.Tensor) -> torch.Tensor:
        ...
        for batch_idx in range(predictions.shape[0]):
            ...
            #fde_loss = f.mse_loss(best_mode_trajectory[-1,:], targets[batch_idx][-1,:])


            # d = torch.norm(best_mode_trajectory - targets[batch_idx], dim=-1)
            # smax = torch.sum(d * torch.exp(0.1 * d)) / torch.sum(torch.exp(0.1 * d))
