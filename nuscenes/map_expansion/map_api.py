...
class NuScenesMap:
    ...
    def render_map_patch(self,
        ...
        return self.explorer.render_map_patch(box_coords, layer_names, alpha, figsize,
                                              render_egoposes_range, render_legend, fig, axes)
    ...
    def render_map_in_image(self,
        ...
        """
        return self.explorer.render_map_in_image(nusc, sample_token, camera_channel=camera_channel, alpha=alpha,
    ...
    def get_layer_record_in_patch(self,
                                  patch_box: Tuple[float, float, float, float],
                                  patch_angle: float,
                                  layer_name: str,
                                  rot_center: Tuple[float, float]=None)->List:
        """
         Retrieve the records of a particular layer within the specified patch.
         :param patch_box: Patch box defined as [x_center, y_center, height, width].
         :param patch_angle: Patch orientation in degrees.
         :param layer_name: name of map layer to be extracted.
         :return: List of layers in a patch box.
         """
        ...
        patch = self.get_patch_coord(patch_box, patch_angle, rot_center=rot_center)
        ...
        if layer_name == 'drivable_area':
            for record in records:
                polygons = [self.extract_polygon(polygon_token) for polygon_token in record['polygon_tokens']]
    ...
    def get_map_geom(self,
                     ...
                     patch_angle: float,
                     layer_names: List[str],
                     rot_center: Tuple[float, float]=None) -> List[Tuple[str, List[Geometry]]]:
        """
        ...
        """
        return self.explorer.get_map_geom(patch_box, patch_angle, layer_names, rot_center=rot_center)
    ...
    def get_next_roads(self, x: float, y: float) -> Dict[str, List[str]]:
        ...
        road_layers = ['road_segment', 'road_block', 'lane', 'stop_line']
    ...
    def get_patch_coord(self, patch_box: Tuple[float, float, float, float],
                        ...
                        rot_center:Tuple[float, float]=None) -> Polygon:
        """
        ...
        if rot_center is None:
            ...
            patch = affinity.rotate(patch, patch_angle, origin=(rot_center[0], rot_center[1]), use_radians=False)
...
class NuScenesMapExplorer:
    ...
    def get_map_geom(self,
        ...
        for layer_name in layer_names:
            layer_geom = self._get_layer_geom(patch_box, patch_angle, layer_name, rot_center=rot_center)
    ...
    def _get_layer_geom(self,
                        ...
                        patch_angle: float,
                        layer_name: str,
                        rot_center:Tuple[float, float]=None) -> List[Geometry]:
        """
        ...
        """
        if layer_name in self.map_api.non_geometric_polygon_layers:
            return self._get_layer_polygon(patch_box, patch_angle, layer_name, rot_center=rot_center)
        elif layer_name in self.map_api.non_geometric_line_layers:
    ...
    def _get_layer_polygon(self,
                           ...
                           patch_angle: float,
                           layer_name: str,
                           rot_center:Tuple[float, float]=None) -> List[Polygon]:
        """
